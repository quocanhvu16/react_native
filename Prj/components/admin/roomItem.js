...
const Home = (props) => {
    ...
    const deleteRoom = async () => {
        const res = await fetch(`https://dev-smarthome.onrender.com/api/room/${room_id}`,
        {
            method: 'DELETE',
            headers: {
        ...
        console.log('Xoa phong: ', text)
        props.setTmp(!props.tmp)
        alert('Xóa phòng thành công');
        setDeleteModal(false);
    }

    const editRoom = async ( desc, name) => {
        const res = await fetch(`https://dev-smarthome.onrender.com/api/room/${room_id}`,
        {
            method: 'PATCH',
            headers: {
        ...
        console.log('Thong tin phong da sua: ', json)
        props.setTmp(!props.tmp)
                ...
                <View style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(0,0,0,0.5)' }}>
                    <View style={{ backgroundColor: 'white', height: '97%', borderTopLeftRadius: 15, borderTopRightRadius: 15}}>
                        <View style={{ flexDirection: 'row', justifyContent:'space-between', margin: 10 }}>
                            <Text style={{ fontWeight: 'bold', color: 'black', fontSize: 18 }}>Chỉnh sửa thông tin phòng</Text>
                            <TouchableOpacity 
            ...
            <TouchableOpacity
                ...
                onPress={() => setDeleteModal(false)}>
                    <TouchableOpacity 
                        ...
                        activeOpacity={1}>
                        <Text style={{ textAlign: 'center', marginTop: 20, marginBottom: 30, fontSize: 18, color: 'black' }}>Bạn có muốn xóa phòng?</Text>
